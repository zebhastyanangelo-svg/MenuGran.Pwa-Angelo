...
export default function OperatorRidersPage() {
  ...
  const getStatusBadge = (status: Rider['status']) => {
    const badges = {
      ...
      inactive: 'bg-cream-100 text-ink-lighter',
    };
    const labels = {
        ...
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-white rounded-xl shadow-sm p-6 animate-pulse">
            <div className="flex items-center mb-4">
              <div className="w-12 h-12 bg-cream-200 rounded-full mr-3" />
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-5 border border-neutral-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-ink-lighter">Total Repartidores</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-neutral-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-ink-lighter">Disponibles</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-neutral-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-ink-lighter">En Entrega</p>
      ...
      {filteredRiders.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-ink-lighter">No hay repartidores en esta categoria</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredRiders.map((rider) => (
            <div key={rider.id} className="bg-white rounded-xl shadow-sm border border-neutral-100 p-6">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 bg-brand-100 rounded-full flex items-center justify-center mr-3">
                  <span className="text-brand-600 font-bold">{getInitials(rider.name)}</span>
                </div>
                <div className="flex-1">
              ...
              {rider.status === 'available' && (
                <button
                  ...
                  className="w-full py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700 transition"
                >
                  Asignar pedido
                </button>
              )}
          ...
          <div className="bg-white rounded-xl max-w-md w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowAssignModal(false)} className="text-neutral-400 hover:text-ink-lighter">
                  <X className="h-5 w-5" />
                </button>
              </div>

              {readyOrders.length === 0 ? (
                <p className="text-center text-ink-lighter py-6">No hay pedidos listos para entregar</p>
              ) : (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-ink-lighter">{order.customer.name}</p>
                          <p className="text-xs text-ink-lighter flex items-center gap-1">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2 border border-neutral-300 text-ink-light rounded-lg text-sm font-medium hover:bg-cream-50"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex-1 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Asignar
                </button>
              </div>
