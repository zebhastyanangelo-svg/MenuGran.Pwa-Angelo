    ...
    <div className="space-y-8">
      <div className="rounded-3xl bg-white p-6 shadow-sm shadow-slate-200">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-ink-lighter">Analytics general</p>
      ...
      {loading && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[...Array(4)].map((_, index) => (
            <div key={index} className="h-32 animate-pulse rounded-3xl bg-cream-100" />
          ))}
        </div>
      )}
      ...
      {!loading && !error && data && (
        <>
            ...
            {metricCards.map((card) => (
              <div key={card.title} className="overflow-hidden rounded-3xl bg-white p-5 shadow-sm shadow-slate-200">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-ink-lighter">{card.title}</p>
                    <p className="mt-3 text-3xl font-semibold text-ink">
                      {typeof card.value === 'number' ? formatCurrency(card.value) : card.value}
                    </p>
                    <p className="mt-2 text-sm text-ink-lighter">{card.caption}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
            <div className="rounded-3xl bg-white p-6 shadow-sm shadow-slate-200">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-ink-lighter">
                ...
                <span className="rounded-full bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-700">
                ...
                {(data.salesByDay ?? []).map((bar) => {
                  ...
                  return (
                    <div key={bar.day} className="flex flex-col items-center gap-3 text-center">
                      ...
                      />
                      <p className="text-xs font-semibold text-ink-light">{bar.day}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm shadow-slate-200">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-ink-lighter">Distribución</p>
                  <p className="mt-2 text-2xl font-semibold text-ink">Métodos de pago</p>
                </div>
                <span className="rounded-full bg-cream-100 px-3 py-2 text-sm font-semibold text-ink-light">
                  {periodOptions.find((o) => o.value === period)?.label}
                </span>
              </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm shadow-slate-200">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-ink-lighter">Top 5 platos</p>
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                <thead>
                  <tr>
                    <th className="px-4 py-3 font-semibold text-ink-lighter">#</th>
                    <th className="px-4 py-3 font-semibold text-ink-lighter">Plato</th>
                    <th className="px-4 py-3 font-semibold text-ink-lighter">Categoría</th>
                    <th className="px-4 py-3 font-semibold text-ink-lighter">Veces pedido</th>
                    <th className="px-4 py-3 font-semibold text-ink-lighter">Total vendido</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {(data.topDishes ?? []).map((dish) => (
                    <tr key={dish.rank} className="hover:bg-cream-50">
                      <td className="px-4 py-4 font-semibold text-ink">{dish.rank}</td>
                      <td className="px-4 py-4 text-ink">{dish.name}</td>
                      <td className="px-4 py-4 text-ink-lighter">{dish.category}</td>
