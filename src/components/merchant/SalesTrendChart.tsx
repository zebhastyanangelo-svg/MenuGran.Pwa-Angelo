...
import type { RevenueDataPoint } from '../../services/merchantAnalyticsService';
...
const WIDTH = 720;
const HEIGHT = 280;
const M = { left: 56, right: 20, top: 24, bottom: 48 };
...
function buildLinePath(points: RevenueDataPoint[], max: number): string {
...
function buildAreaPath(points: RevenueDataPoint[], max: number): string {
  if (points.length === 0) return '';
  const n = points.length;
...
function XAxis({ data }: { data: RevenueDataPoint[] }) {
  const n = data.length;
  if (n === 0) return null;
...
function ChartSkeleton() {
  return (
    <div
      className="space-y-3"
      aria-label="Cargando gráfica de tendencia de ventas"
      role="status"
    >
      <Skeleton className="aspect-[2.6] w-full rounded-lg" />
...
function EmptyState() {
  return (
    <div
      ...
      aria-label="Sin datos de ventas en el rango seleccionado"
      role="status"
    >
...
function useScales(data: RevenueDataPoint[]) {
  return useMemo(() => {
...
export function SalesTrendChart({ data, isLoading, error }: SalesTrendChartProps) {
  ...
  return (
    ...
    >
      <svg
        ...
        role="img"
        aria-label="Tendencia de ingresos diarios por rango de fechas"
      >
        <defs>
        ...
        {data.map((point, i) => {
          ...
          const r = active ? 5 : 4;
          const dotColor = active ? '#065f46' : '#10B981';
          return (
            <g key={point.date}>
              {active && (
                <line
              ...
              <circle
                ...
                onMouseLeave={handleMouseLeave}
              >
                <title>{`Ventas ${point.date}: ${formatCurrency(point.revenue)}`}</title>
              </circle>
            </g>
          );
        })}
